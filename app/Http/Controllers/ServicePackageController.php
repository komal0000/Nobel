...
class ServicePackageController extends Controller
{
   ...
   public function render($type_name, $service_id)
   {
      ...
      if ($type_name == 'Type 1') {
         ...
         Helper::putCache('service.' . $service->slug . '.package', view('admin.template.service.package.type1', compact('packages1', 'packageCategories', 'service_slug'))->render());
         foreach ($packages1 as $package) {
            Helper::putCache('service.' . $service->slug . '.package.' . $package->slug, view('admin.template.service.package.single', compact('package'))->render() );
         }
      } else {
         $packages2 = DB::table('service_packages')->where('type_name', $type_name)->get();
         $service_slug = $service->slug;
         Helper::putCache('service.' . $service->slug . '.package', view('admin.template.service.package.type2', compact('packages2', 'service_slug'))->render());
